...
const AdminDashboard = () => {
    ...
    // State mới cho xác nhận khôi phục giao dịch
    const [restoreConfirm, setRestoreConfirm] = useState({ show: false, transId: null });
    // State mới cho xác nhận khôi phục TẤT CẢ giao dịch
    ...
    const handleRestoreTransaction = async (transId) => {
        setRestoreConfirm({ show: false, transId: null }); // Đóng modal trước
        try {
            const res = await adminApi.restoreTransaction(transId);
            if (res.data.success) {
                showToast(t('notifySuccess'), 'success');
                if (detailModal.user) {
                    fetchUserTransactions(detailModal.user.id, detailModal.userTransCurrentPage, detailModal.deletedStatus, detailModal.userTransFilterType);
                }
                fetchStats(true);
            } else {
                showToast(res.data.message || t('notifyError'), 'error');
    ...
    const handleAutoLogoutTrigger = async () => {
        ...
        try {
            ...
            if (res.data.success) { showToast(t('autoLogoutSuccess'), 'success'); fetchStats(); }
    ...
    const doughnutData = useMemo(() => ({ 
        ...
        datasets: [{ data: [chartStats.incomePerc, chartStats.expensePerc], backgroundColor: ['#10b981', '#f43f5e'], cutout: '60%', borderRadius: 5 }]
    }), [chartStats, t]);

    // Plugin để vẽ phần trăm trực tiếp lên các phân đoạn của biểu đồ Doughnut
    const doughnutLabelsPlugin = useMemo(() => ({
        ...
        afterDraw: (chart) => {
            ...
            chart.getDatasetMeta(0).data.forEach((datapoint, index) => {
                ...
                if (value > 0) { // Chỉ vẽ nếu giá trị > 0
    ...
    // Plugin để vẽ phần trăm trực tiếp sau mỗi thanh trong biểu đồ Bar
    const barLabelsPlugin = useMemo(() => ({
        ...
        afterDraw: (chart) => {
            ...
            chart.getDatasetMeta(0).data.forEach((bar, index) => {
                const value = data.datasets[0].data[index];
                if (value > 0) {
                    // Tự động điều chỉnh vị trí: Hiện bên trong thanh nếu > 12%, ngược lại hiện bên ngoài
                    const isInside = value > 12;
                    ctx.fillStyle = isInside ? '#ffffff' : '#64748b';
                    ctx.font = 'bold 12px Inter, sans-serif';
                    ctx.textAlign = isInside ? 'right' : 'left';
                    ctx.textBaseline = 'middle';
                    const xPos = isInside ? bar.x - 8 : bar.x + 8;
    ...
    // Logic xử lý cuộn đến Request được nhấn từ thông báo
    useEffect(() => {
        if (highlightRequestId && activeTab === 'contacts' && contactRequests.length > 0) {
            const timer = setTimeout(() => {
                ...
                if (element) {
                    ...
                    // Tự động xóa highlight sau 3 giây để người dùng không bị rối
                    setTimeout(() => setHighlightRequestId(null), 3000);
                }
            }, 300); // Đợi bảng render xong
            return () => clearTimeout(timer);
        }
            ...
            <div className="card shadow-sm border-0 rounded-4 overflow-hidden mb-4">
                <div className="card-header bg-white border-0 pt-4 px-4">
                    <div className="d-flex flex-wrap justify-content-between align-items-center gap-3">
                        <div>
                            <h5 className="mb-1 fw-bold text-dark">{t('flowAnalysis')}</h5>
                            <p className="text-muted small mb-0">{selectedRange ? `${selectedRange.label}` : t('loading')}</p>
                        </div>
                        <div className="btn-group btn-group-sm p-1 bg-light rounded-3">
                            {[{m: 'DAILY', l: t('day')}, {m: 'WEEKLY', l: t('week')}, {m: 'MONTHLY', l: t('month')}, {m: 'QUARTERLY', l: t('quarter')}, {m: 'YEARLY', l: t('year')}].map(item => (
                                <button key={item.m} className={`btn border-0 px-3 rounded-2 transition-all ${rangeMode === item.m ? 'bg-white shadow-sm fw-bold text-primary' : 'text-muted'}`} onClick={() => handleModeChange(item.m)}>{item.l}</button>
                            ))}
                        </div>
                    </div>
                                ...
                                <div className="row g-3">
                                    <div className="col-md-4">
                                        <div className="p-3 rounded-4 bg-light border-start border-4 border-success">
                                            <div className="text-muted small mb-1 fw-bold">{t('income')}</div>
                                            <div className="fw-bold text-success fs-4">{chartStats.incomePerc}% </div>
                                        </div>
                                    </div>
                                    <div className="col-md-4">
                                        <div className="p-3 rounded-4 bg-light border-start border-4 border-danger">
                                            <div className="text-muted small mb-1 fw-bold">{t('expense')}</div>
                                            <div className="fw-bold text-danger fs-4">{chartStats.expensePerc}%</div>
                                        </div>
                                    </div>
                                    <div className="col-md-4">
                                        <div className={`p-3 rounded-4 border-start border-4 ${chartStats.netFlow >= 0 ? 'bg-success-subtle border-success' : 'bg-danger-subtle border-danger'}`}>
                                            <div className="text-muted small mb-1 fw-bold">{t('netFlow')}</div>
                                            <div className={`fw-bold fs-4 ${chartStats.netFlow >= 0 ? 'text-success' : 'text-danger'}`}>{chartStats.netFlowPerc}%</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-4 d-flex flex-column align-items-center justify-content-center py-4">
                                <div style={{ height: '280px', width: '100%', position: 'relative' }}>
                                    ...
                                    <div className="position-absolute top-50 start-50 translate-middle text-center">
                                        <div className="text-muted extra-small text-uppercase fw-extrabold" style={{ letterSpacing: '0.05em' }}>{t('totalVolume')}</div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div style={{ height: `${dynamicChartHeight}px` }}>
                                        <Bar 
                                            ...
                                            options={{ 
                                                ...
                                                plugins: { legend: { display: false }, tooltip: { callbacks: { label: (ctx) => ` ${t('weight')}: ${ctx.raw}%` } } },
                                                scales: { 
                                                    x: { 
                                                        min: 0,
                                                        max: 100, 
                                                        grid: { display: true, color: '#f1f5f9' }, 
                                                        ticks: { display: true, callback: (val) => val + '%', color: '#94a3b8', font: { size: 10 } } 
                                                    }, 
    ...
    return (
        <div className="d-flex min-vh-100 bg-light font-inter">
            ...
            {confirmModal.show && (
                <div className="modal fade show d-block" style={{backgroundColor: 'rgba(0,0,0,0.5)'}}><div className="modal-dialog modal-dialog-centered"><div className="modal-content shadow border-0 rounded-4"><div className="modal-body text-center p-5"><i className={`bi ${confirmModal.isLocked ? 'bi-unlock-fill text-success' : 'bi-lock-fill text-danger'} display-1 mb-4`}></i><h4 className="fw-bold mb-3">{confirmModal.isLocked ? t('confirmUnlock') : t('confirmLock')}</h4><p className="text-muted mb-5">{confirmModal.isLocked ? t('confirmDescUnlock') : t('confirmDescUnlock')}</p><div className="d-flex justify-content-center gap-3"><button className="btn btn-light px-5 py-2 rounded-pill fw-bold" onClick={() => setConfirmModal({...confirmModal, show: false})}>{t('cancel')}</button><button className={`btn ${confirmModal.isLocked ? 'btn-success' : 'btn-danger'} px-5 py-2 rounded-pill fw-bold text-white shadow-sm`} onClick={async () => { try { const res = confirmModal.isLocked ? await adminApi.unlockAccount(confirmModal.userId) : await adminApi.lockAccount(confirmModal.userId); if (res.data.success) { showToast(t('notifySuccess'), 'success'); fetchUsers(false); setConfirmModal({show: false}); } else { showToast(res.data.message || t('notifyError'), 'error'); } } catch (e) { const errMsg = e.response?.data?.message || e.response?.data || e.message || t('notifyError'); showToast(errMsg, 'error'); setConfirmModal({show: false}); } }}>{t('confirm')}</button></div></div></div></div></div>
            )}
            {restoreConfirm.show && (
                <div className="modal fade show d-block" style={{backgroundColor: 'rgba(0,0,0,0.5)'}}><div className="modal-dialog modal-sm modal-dialog-centered"><div className="modal-content shadow border-0 rounded-4"><div className="modal-body text-center p-4"><i className="bi bi-arrow-counterclockwise text-primary display-5 mb-3"></i><h5 className="fw-bold">{t('confirmRestore')}</h5><div className="d-flex justify-content-center gap-2 mt-4"><button className="btn btn-sm btn-light px-3 rounded-pill" onClick={() => setRestoreConfirm({show: false, transId: null})}>{t('cancel')}</button><button className="btn btn-sm btn-primary px-3 rounded-pill shadow-sm" onClick={() => handleRestoreTransaction(restoreConfirm.transId)}>{t('confirm')}</button></div></div></div></div></div>
            )}
            {restoreAllConfirm.show && (
                <div className="modal fade show d-block" style={{backgroundColor: 'rgba(0,0,0,0.5)'}}><div className="modal-dialog modal-sm modal-dialog-centered"><div className="modal-content shadow border-0 rounded-4"><div className="modal-body text-center p-4"><i className="bi bi-arrow-counterclockwise text-primary display-5 mb-3"></i><h5 className="fw-bold">{t('confirmRestoreAll')}</h5><p className="text-muted small">{t('confirmRestoreAllDesc')}</p><div className="d-flex justify-content-center gap-2 mt-4"><button className="btn btn-sm btn-light px-3 rounded-pill" onClick={() => setRestoreAllConfirm({show: false, userId: null})}>{t('cancel')}</button><button className="btn btn-sm btn-primary px-3 rounded-pill shadow-sm" onClick={() => handleRestoreAllUserTransactions(restoreAllConfirm.userId)}>{t('confirm')}</button></div></div></div></div></div>
            )}
            {toast.show && (
