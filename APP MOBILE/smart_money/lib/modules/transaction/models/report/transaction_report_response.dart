...
class TransactionReportResponse {
  ...
  final double debtAmount;      // Tổng tiền Đi vay (DEBT_BORROWING = 20)
  final double loanAmount;      // Tổng tiền Cho vay (DEBT_LENDING = 19)
  final double otherAmount;     // Thu nợ - Trả nợ (DEBT_COLLECTION - DEBT_REPAYMENT)
  ...
  factory TransactionReportResponse.fromJson(Map<String, dynamic> json) {
    return TransactionReportResponse(
      ...
      debtAmount: (json['debtAmount'] as num? ?? 0).toDouble(),
      loanAmount: (json['loanAmount'] as num? ?? 0).toDouble(),
      otherAmount: (json['otherAmount'] as num? ?? 0).toDouble(),
    );
  }
}